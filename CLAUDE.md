...
- **Schedule.kt** — today's programme guide per station, dispatched by station id in `ScheduleFetcher`. Sources: NPO sites' keyless JSON (`/api/broadcasts`); BBC's RMS JSON; RadioDNS/SPI XML for WDR (ARD server, fm bearer path) and Deutschlandradio (own server, dab bearer path); VRT MAX's GraphQL API (`vrtnu-api/graphql/v1`, anonymous with `x-vrt-client-name`/`-version` headers; Klara Continuo has no guide); RTBF's open scheduling API (`bff-service.rtbf.be/oaos/v1.6/schedulings`, broadcast day 04:00–04:00 UTC, La Première=6/Musiq3=7); BNR via the VPRO gids JSON (`digitale.vprogids.nl` — BNR's own site is Cloudflare-walled; times are Europe/Amsterdam without offset); RTL France by parsing the server-rendered `rtl.fr/grille/{dd-MM-yyyy}` HTML (programme-card regex). Radio France works via its GraphQL API only when `radiofrance.api.key` is set. The VRT, RTBF, BNR and RTL sources are undocumented/internal — expect breakage and keep failures graceful.

...
- **Schedule.kt** — today's programme guide per station. Three source types, dispatched by station id in `ScheduleFetcher`: NPO sites' keyless JSON (`/api/broadcasts`), BBC's RMS JSON, and RadioDNS/SPI XML for WDR (ARD server, fm bearer path) and Deutschlandradio (own server, dab bearer path). Radio France works via its GraphQL API only when `radiofrance.api.key` is set. VRT/RTBF have no usable guide source (VRT's RadioDNS schedule files are empty — verified 2026-08).
